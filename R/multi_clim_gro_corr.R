...
multi_clim_gro_corr <- function(rwl.group = NULL,
  ...
  clim1 <- moving_win_multi(clim.group,
                            clim.var = clim.var,
                            win.lens = 2:12,
                            agg.fun = agg.fun)
  ...
  if (prewhiten == TRUE) {
    ...
    clim1.mo.split <- lapply(clim1.mo.split, FUN = \(start.mo) {
      ...
      lapply(start.mo.split, FUN = \(this.win) {
        ...
        this.win[,clim.var] <- forecast::auto.arima(this.win[,clim.var],
                                                    seasonal = FALSE) |>
          residuals() |>
          as.numeric()
        this.win
    ...
    rwl.arima.resids <- apply(rwl.group[,!(colnames(rwl.group) %in% "year"), drop = FALSE],
                              MARGIN = 2, FUN = \(this.series) {
                                these.vals <- which(!is.na(this.series))
                                these.resids <- forecast::auto.arima(this.series,
                                                                     seasonal = FALSE) |>
                                  residuals() |>
                                  as.numeric()
                                this.series[these.vals] <- these.resids
                                this.series
                              }, simplify = FALSE) |> do.call(what = "cbind")
  ...
  if (hemisphere == "S") {
    ...
    lag.list <- sapply(-1:max.lag, FUN = \(n.lag) {
      merge(clim.lag.bind, rwl.group, by.x = paste0("lag", n.lag, ".year"), by.y = "year")
    }, simplify = FALSE)
    names(lag.list) <- c(paste0("lag+", -1*(-1:0)), paste0("lag-", 1:max.lag))

  } else { # For northern hemisphere
  ...
  cor.res.df <- mapply(FUN = \(lag.x, which.lag) {
    ...
    this.lag.corr <- lapply(lag.x.mo.split, FUN = \(mo.split) {
      ...
      apply(all.corr.combos, MARGIN = 1, FUN = \(combo) {
        ...
        subs.for.corr <- mo.split[mo.split[,"win.len"] == win.len,
                                  c("year", "month", "win.len",
                                    as.character(series.name), clim.var)] |>
          na.omit()

        # Control for ties by simply removing them when they occur. This should be
        # relatively rare.
        subs.for.corr <- subs.for.corr[which(!duplicated(subs.for.corr[,
                                                                       clim.var])), ]
        subs.for.corr <- subs.for.corr[which(!duplicated(subs.for.corr[,
        ...
        if (corr.method == "pearson") {
          cor.res <- cor.test(subs.for.corr[, clim.var],
                              subs.for.corr[, as.character(series.name)],
                              method = "pearson",
                              alternative = "two.sided")
          data.frame(series = as.character(series.name),
                     start.month = unique(subs.for.corr[,"month"]),
                     win.len = unique(subs.for.corr[,"win.len"]),
                     coef = cor.res[["estimate"]][[1]],
                     p = cor.res[["p.value"]][[1]],
                     dir = ifelse(cor.res[["estimate"]][[1]] < 0, "Neg.", "Pos."),
                     overlap = min(length(na.omit(mo.split[, as.character(series.name)])),
                                   length(na.omit(mo.split[mo.split[,"win.len"] == win.len,
                                                           clim.var]))),
                     corr.method = corr.method)
        } else { # for "spearman" or "kendall" rank methods
          cor.res <- corTESTsrd::corTESTsrd(subs.for.corr[, clim.var],
                                            subs.for.corr[, as.character(series.name)],
                                            method = corr.method,
                                            ...
                                            alternative = "two.sided")
          data.frame(series = as.character(series.name),
                     start.month = unique(mo.split[,"month"]),
                     win.len = unique(subs.for.corr[,"win.len"]),
                     coef = cor.res[["rho"]],
                     p = cor.res[["pval"]],
    ...
    this.lag.corr$lag <- ifelse(substr(unique(which.lag), 5, 5) == "0",
                                "0",
                                substr(unique(which.lag), 4, 5))
    this.lag.corr

  }, lag.x = lag.list, which.lag = names(lag.list),
