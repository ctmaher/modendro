...
read_pos <- function(path = NULL) {
  ...
  out.list <- lapply(pos.files, FUN = \(file) {
    ...
    CR.ver <- gsub(pattern = ".", replacement = "", CR.ver, fixed = TRUE)
    ...
    # Also make sure that the version is at least 7.8
    if (length(grep("DENDRO", raw.input[1])) == 1 &&
      ...
      this.series.list
    } else { # file doesn't meet criteria above for DENDRO files & CooRecorder version

      warning("Some .pos files in path were not DENDRO files OR were not CooRecorder 7.8 or greater and
          were skipped")
  ...
  if (length(out.list[[!is.na(out.list)]]) > 1) { # If many series, bind the ring widths and
