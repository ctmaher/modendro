...
#' of an optimal power for making tree ring series ± homoscedastic. Operationally in the `modendro` package, the final determination
#' of how to transform series is performed in the  \code{\link{pwr_t_rwl}} function, per Cook & Peters (1997).
#'
#' The `universal` and `ID.group.substr` arguments control how many series are included in the spread vs. level estimation,
#' allowing for the same transformation to be performed across all tree ring series within each group (you determine the group). `universal` is a logical switch while
#' `ID.group.substr` is a 2-element numeric that passes to the `start` & `stop` args of \code{\link[base]{substr}} which will act on the series IDs (i.e., the colnames in `rwl`).
#' For example, if the first 3 characters of your series IDs refers to the site where the series were collected, you would specify `ID.group.substr = c(1, 3)`.
#' Or, say the 4th element of the series IDs refers to the species collected, you could specify `ID.group.substr = c(4, 4)` to estimate optimal power by species.
#' This assumes your series IDs contain this information in a systematic way. If not, you could also just create separate rwls for each of the groups you are interested in, set `universal = TRUE` and leave `ID.group.substr` unspecified.
#'
...
#' @param ID.group.substr A numeric vector of length 2 that determines a sub grouping for calculating "universal" optimal power. Passes to \code{\link[base]{substr}} as the start (1st number) and stop (2nd number) args to split the series IDs in your rwl into groups.
#'
#' @return A named numeric vector with the series IDs (colnames) and the estimated optimal power of transformation for each series -or- a numeric vector of length 1.
#'
#' @references
...
find_opt_pwr <- function(rwl, universal = FALSE, ID.group.substr = NULL) {
  ...
  # absolute value of 1st differences
  # Cook & Peters equate this as the standard deviation (it is not the same, but 1st differences are what they use).
  ...
  if (universal == FALSE) {
    ...
    if (is.null(ID.group.substr)) {
      ...
      # Universal estimate with random slope & intercept for each series, and random intercept for each year across all series
      # univ.slopes <- lmer(log10(diff) ~ log10(lmean) + (log10(lmean)|series) + (1|year), data = diff.lmean)
      # univ.slope <- coef(univ.slopes)$series[,"log10(lmean)"] |> median() # The median of all random slopes
