...
plot_n_mon_corr <- function(x = NULL) {
  ...
  lag.levels <- as.numeric(res.agg$lag) |> unique()

  res.agg$lag <- factor(res.agg$lag, levels = lag.levels[order(lag.levels)])
  ...
  out.plot.list <- list(per.plot, mean.plot, res.agg)
